...
Generated by Zander AI using Boris Method`;
      ...
      case 'draft_email': {
        const { to, subject, body, ticketId, tenantId: inputTenantId } = toolInput as {
          to: string;
          subject: string;
          body: string;
          ticketId?: string;
        ...
        console.log(`[Zander Tool] POST ${scheduledUrl} (draft_email, needs approval)`);
        ...
        if (!scheduleRes.ok) {
          ...
          console.log(`[Zander Tool] Draft email error: ${scheduleRes.status} - ${errorText}`);
          return {
            success: true,
            result: {
              message: 'Email draft created for review',
              draft: { to, subject, body: body.substring(0, 200) + '...', status: 'draft' },
...
*— The Zander Platform Team*`;
        ...
        // Create scheduled communication with needsApproval=true
        const scheduledUrl = `${API_URL}/scheduled-communications`;
        console.log(`[Zander Tool] POST ${scheduledUrl} (status update, needs approval)`);
        const scheduleRes = await fetch(scheduledUrl, {
          method: 'POST',
          headers,
          body: JSON.stringify({
            recipientEmail: 'users@notify.zanderos.com',
            recipientName: 'Platform Users',
            type: 'email',
        ...
        return {
          success: true,
          result: {
            message: "Status update saved — it's in your Scheduled queue pending approval",
            scheduledId: scheduled.id,
            status: 'pending',
...
${isTechnical
  ...
  : `Thanks for using Zander! Questions? Reach out to support@zanderos.com`
        ...
        // Create scheduled communication with needsApproval=true
        const scheduledUrl = `${API_URL}/scheduled-communications`;
        console.log(`[Zander Tool] POST ${scheduledUrl} (release notes, needs approval)`);
        const scheduleRes = await fetch(scheduledUrl, {
          method: 'POST',
          headers,
          body: JSON.stringify({
        ...
        return {
          success: true,
          result: {
            message: "Release notes saved — it's in your Scheduled queue pending approval",
            scheduledId: scheduled.id,
            status: 'pending',
            version: versionStr,
            features: featureList.length,
...
*Generated by Zander AI — ${new Date().toLocaleDateString()}*`;
        ...
        // Create scheduled communication for approval queue
        const scheduledUrl = `${API_URL}/scheduled-communications`;
        const scheduleRes = await fetch(scheduledUrl, {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            contactId: null,
            recipientEmail: null,
            recipientName: 'Founder Update Recipients',
            type: 'email',
            subject: `Founder Update — ${period}`,
            body: content,
            scheduledFor: new Date().toISOString(),
            needsApproval: true,
            createdBy: 'zander-ai'
        ...
        if (!scheduleRes.ok) {
          ...
          return {
            success: false,
            error: `Failed to create founder update draft: ${scheduleRes.status} - ${errorText}`
          };
        }
        ...
        return {
          success: true,
          result: {
            message: 'Founder update draft created and queued for approval',
            scheduledId: scheduled.id,
            draft: {
              period,
              content,
            ...
            note: 'Draft saved to approval queue. Review and approve before sending to investors/team.'
          }
        };
      }
...
— The Zander Team`;
        ...
        if (!scheduleRes.ok) {
          ...
          return {
            success: false,
            error: `Failed to create beta announcement draft: ${scheduleRes.status} - ${errorText}`
          };
        }
        ...
        return {
          success: true,
          result: {
            message: 'Beta announcement draft created and queued for approval',
            scheduledId: scheduled.id,
            draft: {
              subject,
              content,
            ...
            note: 'Draft saved to approval queue. Review and approve before sending to beta users. Never auto-sends.'
          }
        };
      }
      ...
      case 'bulk_tenant_message': {
        ...
        const tiers = targetTiers || ['all'];
        const recipientDescription = tiers.includes('all') ? 'All Tenants' : `Tenants: ${tiers.join(', ')}`;
        ...
        const scheduleRes = await fetch(scheduledUrl, {
          ...
          body: JSON.stringify({
            contactId: null,
            recipientEmail: null,
            recipientName: recipientDescription,
            type: 'email',
            subject,
            body: `[BULK MESSAGE - Target: ${recipientDescription}]\n\n${body}`,
            scheduledFor: new Date().toISOString(),
            needsApproval: true,
            createdBy: 'zander-ai'
          })
        ...
        if (!scheduleRes.ok) {
          ...
          return {
            success: false,
            error: `Failed to create bulk message draft: ${scheduleRes.status} - ${errorText}`
          };
        }
        ...
        return {
          ...
          result: {
            ...
            message: 'Bulk message draft created and queued for approval',
            scheduledId: scheduled.id,
            draft: {
              targetTiers: tiers,
              subject,
            ...
            note: 'Draft saved to approval queue. Review carefully before sending. Zander never auto-sends bulk messages.',
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get tenant ID from header (Zander operates cross-tenant, but needs context)
    ...
    // tenantId is optional for Zander (system-wide access) but preferred
    const tenantId = bodyTenantId || tenantHeader || 'system';
